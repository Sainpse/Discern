...
class NeuralNetwork:
    ...
    """
    def train(self, data,Y, cycles, learning_rate):
        self.Y = Y
        for cyc in range(cycles):
            ActivationLearn, caches = self.L_model_forward(data)
            errorRate = self.compute_cost(ActivationLearn,Y)
            print("Accuracy: " + str(100 - (errorRate*100)))
            print("cycle "+ str(cyc))
            grads = self.L_model_backward(ActivationLearn,Y,caches)
